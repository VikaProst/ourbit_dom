...
async function weexPoll(){
  ...
  try{ const r=await fetch("/api/weexdepth?symbol="+encodeURIComponent(S.symbol)).then(x=>x.json());
    if(r&&r.ok&&r.depth&&r.depth.bids&&r.depth.bids.length){ if(r.tick) weexApplyTick(r.tick); if(r.qprec!=null) S.weexQprec=r.qprec; S.depth=r.depth; S._render=true; status("live","WEEX"); }
...
// при СМЕНЕ биржи всегда гасим LIVE (защита: не переносить «включённую торговлю» с одной биржи на другую)
function _resetLiveOnExSwitch(){ try{ if(window.T) T.armed=false; const lb=document.getElementById("livebtn"); if(lb){ lb.checked=false; lb.disabled=true; } const a=document.querySelector(".arm"); if(a) a.style.color=""; }catch(e){} }
// проверить, что ключи WEEX сохранены и счёт читается → тогда разрешить LIVE на WEEX
function weexCheckReady(){
  fetch("/api/weexaccount").then(x=>x.json()).then(r=>{
    if(!S.exWeex) return;
    const lb=document.getElementById("livebtn");
    if(r&&r.ok){ if(window.T) T.connected=true; if(lb) lb.disabled=false; if(typeof refreshAccountWeex==="function") refreshAccountWeex(); }
    else { if(lb) lb.disabled=true; if(typeof log==="function") log("WEEX: впиши ключи (API Key/Secret/Passphrase) и «Сохранить ключ», чтобы торговать","err"); }
  }).catch(()=>{});
}
function setWeexMode(on){
  S.exWeex=!!on;
  _resetLiveOnExSwitch();
  if(on){ if(S.exMexc){ if(typeof _mexcStop==="function") _mexcStop(); S.exMexc=false; } if(_es){ try{_es.close();}catch(e){} } S.depth=null; S.centerS=null; weexResetFlow();   // своя лента WEEX
    _weexStart(); setExBadge("weex"); weexCheckReady(); }
    ...
    setExBadge("ourbit"); if(typeof reflectConnected==="function"){ fetch("/api/state").then(x=>x.json()).then(s=>reflectConnected(s)).catch(()=>{}); } }   // вернулись на Ourbit — восстановить его LIVE-состояние
}
...
function setMexcMode(on){
  S.exMexc=!!on;
  if(typeof _resetLiveOnExSwitch==="function") _resetLiveOnExSwitch();   // на MEXC клик-торговли нет — гасим LIVE
...
function renderFootprint(){
  ...
  // ТФ<1мин (30с=0.5) → строим из тиков; MEXC/WEEX отдают только ленту (нет серверного футпринта) → тоже из тиков; иначе серверные минутные бакеты
  const _pollSrc=(S.exMexc||S.exWeex);
  const fp=(S.cluTF<1||_pollSrc)?buildFpFromTicks(Math.round((S.cluTF||1)*60)):groupFootprint(S.flow.footprint||[], S.cluTF), L=fp.length;
  ...
  { const ce=$("conn-ex"); if(ce) ce.addEventListener("change",applyConnEx); applyConnEx(); buildConnExUI();
    const wb=$("conn-weex-btn"); if(wb) wb.onclick=async()=>{ const st=$("conn-weex-status");
      ...
      try{ const r=await fetch("/api/weexcreds",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({key,secret:sec,passphrase:pas})}).then(x=>x.json());
        if(st){ if(r&&r.ok){ st.textContent=r.tested?"✅ ключ сохранён, авторизация работает — можно включать LIVE":"⚠ сохранён, но баланс не проверился (проверь права ключа на фьючерсы)"; st.style.color=r.tested?"#5ecb7f":"#e6a943"; }
          else { st.textContent="ошибка: "+((r&&r.error)||"нет ответа"); st.style.color="#ef8f8a"; } }
        if(r&&r.ok && S.exWeex && typeof weexCheckReady==="function") weexCheckReady();   // разблокировать галочку LIVE для WEEX
